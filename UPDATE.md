...
### Kibana Deep Link URL Format

The echo client outputs a Kibana URL that deep links to filtered log results. The URL format is:

```
http://localhost:5601/app/discover#/?_g=(...)&_a=(...)
```

**Global state (`_g`)** - Controls time range and refresh:
- `filters:!()` - No global filters
- `refreshInterval:(pause:!t,value:60000)` - Refresh paused, 60s interval
- `time:(from:now-15m,to:now)` - Last 15 minutes

**App state (`_a`)** - Controls query and display:
- `columns:!(message,log.level,service.name)` - Columns to display
- `filters:!()` - No app filters
- `query:(language:kuery,query:'trace.id:"<id>"')` - KQL filter by trace ID

Example URL generated by echo_client.py:
```
http://localhost:5601/app/discover#/?_g=(filters:!(),refreshInterval:(pause:!t,value:60000),time:(from:now-15m,to:now))&_a=(columns:!(message,log.level,service.name),filters:!(),query:(language:kuery,query:'trace.id:"abc123..."'))
```

**Note:** Do not include `dataViewId` in the URL. Kibana requires the data view UUID (not the name), and UUIDs are dynamically generated per installation. Omitting it allows Kibana to use the default data view. If no results appear initially, click "Search entire time range" in Kibana to find logs outside the default time window.
